...
### Problem: Manual PVC Discovery is Tedious and Error-Prone

After `crane export`, users must manually:
1. **Find which PVCs need transfer** - scan YAML files or kubectl get pvc
2. **Understand PVC dependencies** - which Deployments/StatefulSets use which PVCs
3. **Figure out correct transfer commands** - build crane transfer-pvc commands by hand
4. **Handle StatefulSet PVC naming patterns** - calculate PVC names (e.g., data-kafka-0, data-kafka-1, data-kafka-2)
5. **Map storage classes between clusters** - understand source vs destination storage compatibility

**Result:** Time-consuming, easy to miss PVCs, easy to make mistakes.

---

### Solution: Automated Discovery with Multiple Output Formats

`crane transfer-pvc explore` analyzes export directory and generates:

**✅ Ready-to-use commands** (default)
- Copy-paste directly to terminal
- No manual command construction

**✅ Executable script** (--format=script)
- Complete bash script with error handling
- Parallel transfers for StatefulSets
- Save and run: `./transfer-plan.sh`

**✅ Editable instructions file** (--format=instructions)
- YAML file similar to crane transform
- Review, edit, commit to Git
- Execute: `crane transfer-pvc --instructions-file=pvc-instructions.yaml`
...
| Benefit | Description |
|---------|-------------|
| **⏱️ Save Time** | No manual PVC discovery - tool finds everything automatically |
| **✅ No Missed PVCs** | Discovers all PVCs including StatefulSet volumeClaimTemplates |
| **🎯 Correct Commands** | Generates valid crane transfer-pvc commands with all required flags |
| **📊 See Dependencies** | Shows which workloads use which PVCs |
| **🔄 Reusable** | Save instructions file, edit, re-run for incremental syncs |
| **👥 Team Collaboration** | Commit instructions file to Git, review via PR |
| **⚡ Parallel Transfers** | Script format generates parallel transfer commands for StatefulSets |
| **🗺️ Storage Mapping** | Recommendations for storage class mappings between clouds |
...
```bash
# Step 1: Export workloads (existing workflow)
...
# │      --pvc-namespace=myapp ...                 │
# │                                                 │
# │ 2-4. kafka-data-{0,1,2} (300Gi total)         │
# │    for i in 0 1 2; do                         │
# │      crane transfer-pvc \                      │
# │        --pvc-name=kafka-data-$i ... &          │
# │    done; wait                                  │
# └─────────────────────────────────────────────────┘

# Step 3: Copy-paste commands and run!
```
...
# Specify target cluster context (enables validation)
crane transfer-pvc explore \
  --export-dir=myapp/ \
  --target-context=dest-cluster

# What --target-context validates:
# ✅ Target namespace exists
# ✅ Target storage classes are available
# ✅ Warns about missing storage classes
# ✅ Suggests compatible storage class mappings
...
# Generate migration instructions file (reusable, version-controllable)
crane transfer-pvc explore \
  --export-dir=myapp/ \
  --output=pvc-instructions.yaml \
  --format=instructions

# Execute instructions
crane transfer-pvc --instructions-file=pvc-instructions.yaml
...
| `--target-context` | string | Target cluster context (optional, enables validation) | - |
| `--output` | string | Output file path (stdout if not specified) | stdout |
| `--format` | string | Output format: `text`, `script`, `instructions`, `json` | `text` |
...
3. Analyze relationships
    ...
    ↓
4. (Optional) Validate against target cluster
    │   IF --target-context provided:
    ├── Connect to target cluster
    ├── Check namespace exists
    ├── List available storage classes
    ├── Compare source vs target storage classes
    └── Generate warnings/recommendations
    ↓
5. Generate transfer plan
    ├── Single PVCs → individual commands
    ├── StatefulSet PVCs → batch commands
    ├── Add storage class mapping flags (if needed)
    └── Add recommendations
    ↓
6. Output (text/script/instructions/json)
```
...
type Explorer struct {
    ...
    TargetContext    string  // Optional - if provided, validates against target cluster
    StorageClassMap  map[string]string
    TargetClient     client.Client  // Set when TargetContext provided
}
...
type TransferPlan struct {
    ...
    Validation        *ValidationResult  // nil if --target-context not provided
}
...
type ValidationResult struct {
    ...
    MissingStorageClasses []StorageClassMapping
    Warnings              []string
}

type StorageClassMapping struct {
    SourceClass      string
    TargetClass      string  // empty if not found
    Suggested        []string  // suggested compatible classes
}
...
func (e *Explorer) Discover() (*TransferPlan, error) {
    ...
    // 6. (Optional) Validate against target cluster
    if e.TargetContext != "" && e.TargetClient != nil {
        validation, err := e.validateTargetCluster(plan)
        if err != nil {
            // Non-fatal - just log warning
            plan.Recommendations = append(plan.Recommendations, Recommendation{
                Type:    "warning",
                Message: fmt.Sprintf("Target cluster validation failed: %v", err),
            })
        } else {
            plan.Validation = validation
            
            // Add storage class mapping recommendations
            for _, missing := range validation.MissingStorageClasses {
                if len(missing.Suggested) > 0 {
                    plan.Recommendations = append(plan.Recommendations, Recommendation{
                        Type:    "storage-class",
                        Message: fmt.Sprintf("Map %s → %s", missing.SourceClass, missing.Suggested[0]),
                    })
                }
            }
        }
    ...
    plan.Recommendations = append(plan.Recommendations, e.generateRecommendations(plan)...)
    
    return plan, nil
}

// validateTargetCluster queries target cluster to validate migration readiness
func (e *Explorer) validateTargetCluster(plan *TransferPlan) (*ValidationResult, error) {
    ...
    ns := &corev1.Namespace{}
    err := e.TargetClient.Get(context.TODO(), client.ObjectKey{Name: plan.Namespace}, ns)
    if err != nil {
        result.NamespaceExists = false
        result.Warnings = append(result.Warnings, 
            fmt.Sprintf("Namespace '%s' does not exist in target cluster", plan.Namespace))
    } else {
    ...
    err = e.TargetClient.List(context.TODO(), scList)
    if err != nil {
        return nil, fmt.Errorf("failed to list storage classes: %w", err)
    }
    
    for _, sc := range scList.Items {
        result.AvailableStorageClasses = append(result.AvailableStorageClasses, sc.Name)
    }
    
    // 3. Check source storage classes exist in target
    sourceClasses := make(map[string]bool)
    for _, pvc := range plan.PVCs {
        sourceClasses[pvc.StorageClass] = true
    }
    
    for sourceClass := range sourceClasses {
        found := false
        for _, targetClass := range result.AvailableStorageClasses {
        ...
        if !found {
            // Suggest compatible storage class
            suggested := e.suggestStorageClass(sourceClass, result.AvailableStorageClasses)
            result.MissingStorageClasses = append(result.MissingStorageClasses, StorageClassMapping{
...
// suggestStorageClass recommends compatible storage classes
func (e *Explorer) suggestStorageClass(sourceClass string, available []string) []string {
    suggestions := []string{}
    
    // Simple heuristic based on common patterns
    // AWS → GCP
    if sourceClass == "gp2" || sourceClass == "gp3" {
        for _, sc := range available {
            if sc == "standard-rwo" || sc == "pd-standard" || sc == "pd-balanced" {
    ...
    if sourceClass == "gp2" || sourceClass == "gp3" {
        for _, sc := range available {
            if sc == "managed-premium" || sc == "managed-standard" {
                suggestions = append(suggestions, sc)
            }
        }
    }
    
    // If no specific suggestions, return all available
...
### Example 2: With Target Cluster Validation

**Scenario:** Migrating from AWS (gp2 storage class) to GCP (needs standard-rwo)
...
================================================================================
PVC Transfer Plan for namespace: myapp
Source cluster: aws-cluster
Target cluster: gcp-cluster (connected ✓)
================================================================================

✓ Validated target namespace 'myapp' exists
✓ Validated target cluster connectivity
...
1. postgres-data (50Gi, storageClass: gp2)
   ...
   ⚠️  STORAGE CLASS VALIDATION:
   Source storage class: gp2
   Target cluster available classes:
     • standard-rwo (recommended - similar to gp2)
     • premium-rwo
     • balanced-rwo
   
   ⚠️  Storage class 'gp2' not found in target cluster!
   
   Suggested command with mapping:
   
   crane transfer-pvc \
     --source-context=aws-cluster \
     --destination-context=gcp-cluster \
     --pvc-name=postgres-data \
     --pvc-namespace=myapp \
     --dest-storage-class=standard-rwo \
     --verify
...
✅ Target namespace exists: myapp
❌ Storage class compatibility issues: 1
   • gp2 → Suggest mapping to: standard-rwo

Recommended actions:
1. Review storage class mappings above
2. Create PVCs in target cluster with correct storage class
3. Run transfer commands with --dest-storage-class flag
```
...
transfers:
  ...
  - pvc: postgres-data
    ...
    # method: export
    # destination:
    #   path: ./pvc-backups/postgres-data.tar
```

**Execute instructions:**
```bash
# Review and edit instructions
vim pvc-migration.yaml

# Execute transfers based on instructions
